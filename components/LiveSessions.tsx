...
export default function LiveSessions() {
  return (
    <section className="py-24 w-full text-white overflow-hidden relative bg-[#0b1f3d]">
      <div className="absolute w-[400px] h-[400px] bg-sky-400/18 blur-[120px] top-10 right-[-100px] rounded-full pointer-events-none" />
      <div className="absolute w-[300px] h-[300px] bg-blue-300/18 blur-[100px] bottom-10 left-[-50px] rounded-full pointer-events-none" />
      ...
      <div className="relative z-10 flex gap-6 overflow-x-auto pb-10 pt-4 px-6 md:px-12 xl:px-[calc((100vw-1300px)/2+48px)] snap-x snap-proximity scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {sessions.map((session, index) => (
          <motion.div 
            ...
            key={session.id} 
            className="min-w-[320px] md:min-w-[380px] snap-center bg-[#10284c]/75 backdrop-blur-xl rounded-[2rem] border border-white/10 shadow-2xl overflow-hidden flex-shrink-0 group hover:border-white/30 transition-all duration-300"
          >
            {/* Thumbnail Gradient */}
            ...
            <div className="p-7">
              <p className="text-sm text-cyan-200 font-semibold mb-3 tracking-wide uppercase">{session.date}</p>
              ...
              <p className="text-slate-200 font-medium pb-5 border-b border-white/10 text-sm">Hosted by {session.host}</p>
